...
namespace Diploma
{
    public partial class PCIeX4SimulationPage : UserControl
    {
        // link capabilities
        private const int LaneWidth = 4;
        private const double Gtps = 8.0;        // PCIe 3.0 raw
        private const double EffectiveGBs = 3.94;
        ...
        private async void StartTransfer_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (flowControlCredits > 0)
            {
                ...
                await AnimatePacketAsync(430, 430, 118, 207, "Memory Read TLP", Colors.Yellow, isCPUtoGPU: true);
                await AnimatePacketAsync(430, 430, 263, 382, "Memory Read TLP", Colors.Yellow, isCPUtoGPU: true);
                flowControlCredits--;
                CreditsIndicator.Text = $"Credits: {flowControlCredits}";
                UpdateStatusLog($"GPU checks credits: {flowControlCredits} remaining");
                await AnimatePacketAsync(430, 430, 382, 263, "Completion TLP", Colors.LightGreen, isCPUtoGPU: false);
                await AnimatePacketAsync(430, 430, 207, 118, "Completion TLP", Colors.LightGreen, isCPUtoGPU: false);
                UpdateStatusLog($"GPU sends ACK DLLP for Seq: {sequenceNumber}");
                await AnimatePacketAsync(430, 430, 382, 263, "ACK DLLP", Colors.LightBlue, isCPUtoGPU: false);
                await AnimatePacketAsync(430, 430, 207, 118, "ACK DLLP", Colors.LightBlue, isCPUtoGPU: false);
                sequenceNumber++;
                flowControlCredits++;
                CreditsIndicator.Text = $"Credits: {flowControlCredits}";
        ...
        private async void IntroduceError_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (flowControlCredits > 0)
            {
                ...
                await AnimatePacketAsync(430, 430, 118, 207, "Memory Read TLP (Error)", Colors.Red, isCPUtoGPU: true);
                await AnimatePacketAsync(430, 430, 263, 382, "Memory Read TLP (Error)", Colors.Red, isCPUtoGPU: true);
                flowControlCredits--;
                CreditsIndicator.Text = $"Credits: {flowControlCredits}";
                UpdateStatusLog($"GPU detects CRC error, sends NAK DLLP");
                await AnimatePacketAsync(430, 430, 382, 263, "NAK DLLP", Colors.Orange, isCPUtoGPU: false);
                await AnimatePacketAsync(430, 430, 207, 118, "NAK DLLP", Colors.Orange, isCPUtoGPU: false);
        ...
        private async void MemoryWrite_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (flowControlCredits > 0)
            {
                ...
                await AnimatePacketAsync(430, 430, 118, 207, "Memory Write TLP", Colors.Purple, isCPUtoGPU: true);
                await AnimatePacketAsync(430, 430, 263, 382, "Memory Write TLP", Colors.Purple, isCPUtoGPU: true);
                flowControlCredits--;
                CreditsIndicator.Text = $"Credits: {flowControlCredits}";
                UpdateStatusLog($"GPU checks credits: {flowControlCredits} remaining");
                UpdateStatusLog($"GPU sends ACK DLLP for Seq: {sequenceNumber}");
